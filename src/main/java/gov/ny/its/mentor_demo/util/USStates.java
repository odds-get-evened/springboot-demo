...
public enum USStates {
	...
	USStates(String fn, String ansi, String iso) {
		this.fullname = fn;
		this.ANSI = ansi;
		this.ISO = iso;
	}
	 ...
	 */
	public static USStates parse(String input) {
		if (null == input) {
			return null;
		}
		input = input.trim();
		for (USStates state : values()) {
			if (state.fullname.equalsIgnoreCase(input)    ||
					state.ANSI.equalsIgnoreCase(input) ||
					state.ISO.equalsIgnoreCase(input)) {
				return state;
			}
		}
		return null;
